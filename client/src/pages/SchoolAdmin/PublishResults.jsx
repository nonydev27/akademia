...
export default function PublishResults() {
  ...
  async function publishOne(student) {
    ...
    try {
      ...
      if (status === 'RELEASED') {
        ...
        toast(`${student.fullName}'s report card WITHHELD — GHS ${balance?.toFixed(2)} outstanding`, { icon: '⚠️' });
  ...
  function triggerConfetti() {
    const $ = window.$;
    if (!$) return;
    // Animate 15 confetti pieces
    for (let i = 0; i < 15; i++) {
      const colors = ['#f59e0b', '#3b82f6', '#a78bfa', '#22c55e', '#ef4444'];
      const el = $('<div>').css({
        position: 'fixed',
      ...
      el.animate({
        ...
        top: `${Math.random() * 100}%`,
        opacity: 0,
      }, 1000 + Math.random() * 800, () => el.remove());
    }
  }
  ...
  const columns = [
    ...
    { key: 'id', label: 'Fee Balance',
      render: (id) => {
        ...
        return (
          <span className={`font-bold text-sm ${b > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
            {b > 0 ? `GHS ${b.toFixed(2)} owed` : '✅ Cleared'}
          </span>
        );
      }},
    ...
    <div className="space-y-6" ref={confettiRef}>
        ...
        <div className="card p-5 animate-fade-in-up">
          <div className="flex justify-between mb-4">
            ...
            <div className="flex gap-3 text-xs text-slate-500">
              <span>
                ✅ {Object.values(rcMap).filter((r) => r?.status === 'RELEASED').length} released
              </span>
              <span>
                ⚠️ {Object.values(rcMap).filter((r) => r?.status === 'WITHHELD').length} withheld
              </span>
            </div>
          </div>
          <DataTable
      ...
      {result && (
        <Modal
          ...
          title={result.type === 'released' ? '🎉 Report Card Released!' : '⚠️ Report Card Withheld'}
          size="sm"
        >
